...
const getOptimalColor = (
  ...
  const prevColor = blocks[insertIndex]?.color;
  const nextColor = blocks[insertIndex + 1]?.color;

  // Собираем использованные цвета вокруг места вставки
  const nearbyColors = new Set<string>();
  if (prevColor) nearbyColors.add(prevColor);
  if (nextColor) nearbyColors.add(nextColor);

  // Ищем наименее использованный цвет, который не совпадает с соседними
  const colorUsage = DEFAULT_COLORS.map((color) => ({
    color,
    isNearby: nearbyColors.has(color),
    usage: blocks.filter((b) => b.color === color).length,
  }));

  // Сначала ищем цвета, которых нет рядом
  const availableColors = colorUsage.filter((c) => !c.isNearby);
  if (availableColors.length > 0) {
    availableColors.sort((a, b) => a.usage - b.usage);
    return availableColors[0].color;
  }

  // Если все цвета используются рядом, берем наименее использованный
...
function getMinWidth(total: number, blockWidth: number, minWidth: number) {
  ...
  const targetMin =
    blockWidth > minWidth ? minWidth : Math.max(5, blockWidth * 0.5);
...
export function useFunnel(initialBlocks?: IFunnelBlock[]) {
  ...
  const blocks = useMemo(
    () => Object.values(blocksMap).sort((a, b) => a.order - b.order),
    [blocksMap]
  );
  ...
  const OnToggleStates = useCallback(
    ({
      showNumber,
      colored,
      showDescription,
  ...
  const addBlockAfter = (afterId?: string) => {
    ...
    const newBlock: IFunnelBlock = {
      ...
      title: "Новый шаг",
      description: "",
      color: getOptimalColor(blocks, idx),
    };
